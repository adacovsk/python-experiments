...
class Sharpener:
    ...
    def sharpen_frame(self, frame):
        """
        Sharpen a video frame (numpy array in BGR format)
        Args:
            frame: numpy array (H, W, 3) in BGR format
        Returns:
            numpy array: Sharpened frame in BGR format
        """
        # Convert BGR to RGB
        frame_rgb = frame[:, :, ::-1]
        ...
        sharpened_np = sharpened.squeeze(0).permute(1, 2, 0).cpu().numpy()
        sharpened_np = (sharpened_np * 255).astype(np.uint8)

        # Convert RGB back to BGR
        return sharpened_np[:, :, ::-1]
