...
class LowMemoryVideoProcessor:
    def __init__(self, weights_path, device='cpu'):
        ...
        # Set small batch size for limited RAM
        self.batch_size = 1
    ...
    def enhance_video(self, input_path, output_path, target_height=720):
        """Process video with disk-based frame storage for low memory usage"""
    ...
    def _create_video_from_frames(self, temp_dir, output_path, fps, target_height):
        command = [
            ...
            '-crf', '18',
            '-vf', f'scale=-1:{target_height}',  # Scale to target height while maintaining aspect ratio
            '-pix_fmt', 'yuv420p',
            '-y',
            output_path
        ]
...
class EnhancedVideoProcessor(LowMemoryVideoProcessor):
...
def main():
    try:
        ...
        processor.enhance_video(
            input_path="20130809_023600.wmv",
            output_path="enhanced_output.mp4",
            target_height=720,
            target_fps=20  # Target FPS (will interpolate frames to reach this)
        )
    except Exception as e:
        print(f"Error: {str(e)}")
        import sys
